...
export function OrderPieChart() {
  ...
  const[response,setResponse]=React.useState<Array<Record<string,any>>>([])
  ...
  return (
    <CardContent>
        ...
        <div>
            <ul className=" text-sm flex flex-col gap-2">{chartData.map((item,index)=>(
              <li key={index} className=" flex items-center w-full justify-between">
                  <div>{item.status}</div>
                  <div style={{background:item.fill}} className={`w-6 h-6 flex items-center justify-center rounded-full text-white`}>{item.orders}</div>
              </li>
            ))}</ul>
        </div>
    </CardContent>
