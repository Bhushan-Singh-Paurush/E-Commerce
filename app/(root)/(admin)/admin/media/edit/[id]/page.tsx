...
const EditPage = () => {
  ...
  const{loading,
        refetchFunction,
        file,
        error,}=useFetch({url:`/api/media/edit/${id}`})
  ...
  return (
     ...
     { loading ? <LoadingGIF/> : error ? <div className=' flex w-full h-full items-center justify-center text-red-500'>{error.message}</div> : 
     
     <CardContent>
         <Image src={file?.data?.secure_url || ImagePlaceholder} width={150} height={150} alt='Image'/>
          <Form {...form}>
               <form onSubmit={form.handleSubmit(onSubmit)} className="w-full flex flex-col gap-4 items-start">
                 ...
                 />
                  ...
                  <LoadingBtn className=' w-fit hover:cursor-pointer' text='Update' type='submit' disable={updateLoading}/>
