        ...
        <div>
          {data && data.pages[0].allReviews.length > 0 && (
            <div className=" flex flex-col gap-4">
              <div className=" text-2xl font-semibold">{data.pages[0].allReviews.length} Reviews</div>
              <div>{data.pages[0].allReviews.map(
                (item: Record<string, any>, index: any) => (
                  
                    <div key={index} className=" flex items-start gap-2">
                        <Image src={item?.avatar?.url || image_placeholder} width={50} height={50} className=" w-10 h-10  rounded-full" alt="user_img"/>
                        <div>
                              <div className=" font-semibold capitalize">{item.title}</div>
                              <div >{item.userName} <span className=" text-muted-foreground">{dayjs(item.createdAt).fromNow()}</span></div>
                              <div className=" mt-2 text-muted-foreground">{item.review}</div>
                        </div>
