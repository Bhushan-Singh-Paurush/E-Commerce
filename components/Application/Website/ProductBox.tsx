...
const ProductBox = ({product}:{product:Record<string,any>}) => {
        ...
        <Link href={WEBSITE_PRODUCT_DETAILS(product.slug)}>
                 <div  className=' flex flex-col border-[1px] border-border rounded-lg hover:shadow-xl '>
                          ...
                          <div className=' p-2 flex flex-col gap-2 border-t-[1px] border-border'>
                                     ...
                                     <div><span className=' line-through text-muted-foreground'>{parseInt(product.mrp).toLocaleString("en-IN",{style:"currency",currency:"INR"})} </span> {parseInt(product.sellingPrice).toLocaleString("en-IN",{style:"currency",currency:"INR"})}</div>
                          </div>
